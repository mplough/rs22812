...
class MyFrame(wx.Frame):
    def __init__(self, parent, id):
        wx.Frame.__init__(
            self, parent, id, "meter.py", size=(settings["frame size horiz"], settings["frame size vert"])
        )
        self.interval = 0
    ...
    def tips(self):
        ...
        self.tips["Start"] = [
            BT.BalloonTip(
                topicon=None, toptitle="Start button", message=TipFormatter(dict["Start"]), shape=shape, tipstyle=style
            ),
            self.start,
        ]
        self.tips["Set"] = [
            BT.BalloonTip(
                topicon=None, toptitle="Set button", message=TipFormatter(dict["Set"]), shape=shape, tipstyle=style
            ),
            self.set,
        ]
        self.tips["Textbox"] = [
        ...
        self.tips["Reading"] = [
            BT.BalloonTip(
                topicon=None, toptitle="Reading", message=TipFormatter(dict["Reading"]), shape=shape, tipstyle=style
            ),
            self.reading,
        ]
    ...
    def sampling(self):
        ...
        self.rate = wx.TextCtrl(self.panel, -1, settings["default sample rate"], size=(50, 20))
        # Choice box for units
        units = wx.StaticText(self.panel, -1, "units", style=wx.ALIGN_CENTER)
        self.unit_choices = ["seconds", "minutes", "hours"]
        self.units = wx.ListBox(self.panel, -1, (20, 20), (80, 50), self.unit_choices, wx.LB_SINGLE)
    ...
    def time_stamp(self):
        ...
        self.timestamp = wx.StaticText(
            self.panel,
            -1,
            "",
            size=(settings["frame size horiz"], settings["time font size"] * font_factor),
        ...
        self.timestamp.SetFont(wx.Font(settings["time font size"], wx.DEFAULT, wx.NORMAL, wx.BOLD))
        self.sizer.Add(self.timestamp, 0, wx.EXPAND)

    def reading(self):
        # Static text control for the reading
        self.reading = wx.StaticText(
            self.panel,
            -1,
            "0.0 V",
            size=(settings["frame size horiz"], settings["reading font size"] * font_factor),
            style=wx.ALIGN_CENTER | wx.ST_NO_AUTORESIZE,
        )
        self.reading.SetBackgroundColour(settings["reading bg color"])
        self.reading.SetForegroundColour(settings["reading fg color"])
        self.reading.SetFont(wx.Font(settings["reading font size"], wx.DEFAULT, wx.NORMAL, wx.BOLD))
        self.sizer.Add(self.reading, 0, wx.EXPAND)
        self.sizer.Add((1, 5))

    def tips_button(self):
        self.tips_button = wx.ToggleButton(self.panel, -1, "Tips", style=wx.BU_EXACTFIT)
    ...
    def GetReading(self):
        ...
        if self.running:
            ...
            reading_string = self.CurrentTime() + " [%d] " % self.count + str(reading_tuple)
            self.count += 1
    ...
    def OnSet(self, event):
        ...
        try:
            ...
            wx.MessageBox("'%s' is an improper sample rate" % self.rate.GetLabel())
            return
        # Get units
        ...
        if event:
            ...
            D("Simulating " + s)
        self.log.write("  New sampling interval {:g} s {}\n".format(self.interval, self.CurrentTime()))
